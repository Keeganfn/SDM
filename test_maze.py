    ...
    for x in range(m.cols):
        for y in range(m.rows):
            ...
            assert m.state_from_index(
                m.index_from_state(state)) == state, "Mapping incorrect for state: {state}".format(
                state=state)
    ...
    for x in range(m2.cols):
        for y in range(m2.rows):
            for dx in range(3):
                for dy in range(3):
                    state = (x, y, dx, dy)
                    assert m2.state_from_index(
                        m2.index_from_state(state)) == state, "Mapping incorrect for state: {state}".format(
                        state=state)
